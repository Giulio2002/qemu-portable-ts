...
import {
  QemuBinaryNotFoundError,
  QemuInvalidCommandError,
...
test("getHostPlatform maps known platform/arch combinations", () => {
  ...
  assert.equal(getHostPlatform({ platform: "win32", arch: "arm64" }), "win32-arm64");
});
...
test("resolveQemuBinary rejects command names that escape bin/ (traversal)", () => {
  // No allowlist, but a command must still name a binary INSIDE bin/ — a path
  // that traverses out is refused so it can't become arbitrary-binary execution.
  for (const evil of ["../../../../bin/sh", "/bin/sh", "sub/qemu-img", "..", ""]) {
    assert.throws(
      () =>
        resolveQemuBinary(evil, {
          platform: "linux-x64",
          searchPaths: ["/nonexistent-search-root"],
        }),
      (err: Error) => {
        assert.ok(err instanceof QemuInvalidCommandError, `expected reject for ${JSON.stringify(evil)}`);
        assert.equal(err.code, "ERR_QEMU_INVALID_COMMAND");
        return true;
      }
    );
  }
...
test("resolveQemuBinary allows any bare command name (no curated allowlist)", () => {
  // qemu-io is not one of the typed commands, but if a package ships it the
  // resolver resolves it. Absent here -> not-found, crucially NOT a rejection.
  const fake = makeFakePlatformPackage("linux-x64", ["qemu-img"]);
  assert.throws(
    () =>
      resolveQemuBinary("qemu-io", {
        platform: "linux-x64",
        searchPaths: [fake.searchPath],
      }),
    (err: Error) => {
      assert.ok(err instanceof QemuBinaryNotFoundError);
      assert.ok(!(err instanceof QemuInvalidCommandError));
      return true;
    }
  );

  const withIo = makeFakePlatformPackage("linux-x64", ["qemu-io", "qemu-nbd"]);
  const resolved = resolveQemuBinary("qemu-io", {
    platform: "linux-x64",
    searchPaths: [withIo.searchPath],
  });
  assert.ok(resolved.path.endsWith(join("bin", "qemu-io")));
});

test("listAvailableBinaries lists every qemu-* binary present in bin/", () => {
